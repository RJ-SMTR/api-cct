...
export class RelatorioConsolidadoRepository {
  ...
  private getQueryConsorcio(dataInicio: string, dataFim: string, pago?: boolean, valorMin?: number, valorMax?: number, nomeConsorcio?: string[], emProcessamento?: boolean) {
    let query = ` select * from ( `;
    query = query + ` select cs."consorcio" nomeFavorecido,sum(cs."valor_agrupado")::float valor from ( `;
    query =
      query +
  ...
  private getQueryOperadores(dataInicio: string, dataFim: string, pago?: boolean, valorMin?: number, valorMax?: number, favorecidoNome?: string[], emProcessamento?: boolean) {
    let query = ` select * from ( `;
    query = query + ` select cs."favorecido" nomeFavorecido,sum(cs."valor_agrupado")::float  valor from ( `;
    query =
      query +
  ...
  public async findConsolidado(args: IFindPublicacaoRelatorio): Promise<RelatorioConsolidadoDto[]> {
    ...
    if ((args.aPagar === undefined || args.aPagar === false) && (args.consorcioNome !== undefined || args.favorecidoNome === undefined)) {
      queryConsorcio = this.getQueryConsorcio(args.dataInicio.toISOString().slice(0, 10), args.dataFim.toISOString().slice(0, 10), args.pago, args.valorMin, args.valorMax, args.consorcioNome, args.emProcessamento);
    }
    ...
    if ((args.aPagar === undefined || args.aPagar === false) && (args.consorcioNome === undefined || args.favorecidoNome !== undefined)) {
      queryOperadores = this.getQueryOperadores(args.dataInicio.toISOString().slice(0, 10), args.dataFim.toISOString().slice(0, 10), args.pago, args.valorMin, args.valorMax, args.favorecidoNome, args.emProcessamento);
    }
