  ...
  async sincronizarEAgruparOrdensPagamento() {
    ...
    if (locked) {
      try {
        ...
        await this.ordemPagamentoService.sincronizarOrdensPagamento(new Date('2026-02-13'), new Date('2026-02-16'), consorciosEModais);
        ...
        await this.ordemPagamentoAgrupadoService.prepararPagamentoAgrupados(new Date('2026-02-13'), new Date('2026-02-16'), new Date('2026-02-17'), pagadorKey, CronJobsService.MODAIS);
        this.logger.log('Tarefa finalizada com sucesso.', METHOD);
      } catch (error) {
        this.logger.error(`Erro ao executar tarefa, abortando. - ${error}`, error?.stack, METHOD);
      } finally {
