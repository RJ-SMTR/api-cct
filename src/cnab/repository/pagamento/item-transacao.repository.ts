...
export class ItemTransacaoRepository {
  ...
  public async update(id: number, dto: DeepPartial<ItemTransacao>) {
    const updateResult = await this.itemTransacaoRepository.update(
      { id: id },
      dto,
    );
    const updated = await this.itemTransacaoRepository.findOneOrFail({
      where: {
        id: Number(updateResult.generatedMaps[0].id),
      }
    });
    return updated;
  }
  ...
  public async saveManyIfNotExists(
    ...
    if (existing.length === dtos.length) {
      logWarn(
        this.logger,
        `${existing.length}/${dtos.length} DetalhesB já existem, nada a fazer...`,
      );
    } else if (existing.length) {
      logWarn(
        this.logger,
        `${existing.length}/${dtos.length} DetalhesB já existem, ignorando...`,
      );
      return [];
    }
    // Save new
