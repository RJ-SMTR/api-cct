...
import { ADMIN_EMAIL, ADMIN_PASSWORD, APP_URL, LICENSEE_CASE_ACCENT, LICENSEE_CPF_PERMIT_CODE, MAILDEV_URL, TO_UPDATE_PERMIT_CODE } from '../utils/constants';
import { parseStringUpperUnaccent } from 'src/utils/string-utils';

describe('Admin Financeiro managing Lancamento (e2e)', () => {
  const app = APP_URL;
  ...
  describe('Manage users', () => {
     ...
     */ async () => {
      // Arrange
      await request(app).get('/api/v1/test/users/reset-testing-users').expect(200);
      ...
      const args = [
        {
          ...
          expect: (body: any) => expect(body.data.some((i: any) => i.fullName === LICENSEE_CASE_ACCENT)).toBeTruthy(),
        },
        {
          filter: { permitCode: licensee.permitCode },
          expect: (body: any) => expect(body.data.some((i: any) => i.permitCode === LICENSEE_CPF_PERMIT_CODE)).toBeTruthy(),
        },
        {
        ...
        },
        {
          ...
          expect: (body: any) => expect(body.data.some((i: any) => i.fullName === 'Queued user')).toBeTruthy(),
        },
        {
          filter: { name: licenseePartOfName, inviteStatus: 'sent' },
          expect: (body: any) => expect(body.data.some((i: any) => i.fullName === 'Sent user')).toBeTruthy(),
        },
        {
          filter: { name: licenseePartOfName, inviteStatus: 'used' },
          expect: (body: any) => expect(body.data.some((i: any) => i.fullName === 'Used user')).toBeTruthy(),
        },
      ];
  ...
  describe('Upload users', () => {
     ...
     */ async () => {
      ...
      users = await request(app)
        ...
        .expect(({ body }) => {
          expect(body.data?.length).toBe(1);
          expect(body.data[0]?.fullName).toEqual(parseStringUpperUnaccent(uploadUsers[0].nome));
     ...
     */ async () => {
      ...
      forgotLocalDate.setMinutes(forgotLocalDate.getMinutes() + global.__localTzOffset);

      newUser.hash = await request(MAILDEV_URL)
        .get('/email')
        .then(({ body }) =>
          (body as any[])
            .filter((letter: any) => letter.to[0].address.toLowerCase() === newUser.email.toLowerCase() && /.*conclude\-registration\/(\w+).*/g.test(letter.text) && differenceInSeconds(forgotLocalDate, new Date(letter.date)) <= 10)
            .pop()
